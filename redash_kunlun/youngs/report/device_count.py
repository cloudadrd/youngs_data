...
def execute_sql(day):
    sql_str="""
    ALTER TABLE real_bi.fenix_device DELETE where day = '%s'
    """%day

    client.execute(sql_str)


def get_mysql(day,date):
    ...
    sql_kunlun = """
    select REPLACE(day,'-','')  as day ,case when platform = '1' then 'Android' when platform = '2' then 'iOS' end platform,country,source,label,slot_id,device_count from cloudmobi_yibu_setting  where day = '%s'
""" %day
    cur.execute(sql_kunlun)
    #print(cur.fetchall())
    buzz_df = pd.DataFrame(list(cur.fetchall()),columns=['day','platform','country','source','label','slot_id','device_count'])
    return buzz_df
...
if __name__ == '__main__':
    ...
    clickhouse_client = clickhouse_util()
    client=clickhouse_client.return_client()
    ys_day = datetime.datetime.today()
#- datetime.timedelta(days=1)
    ...
    df_result = get_mysql(day_format,day)
    print(df_result)

    type_dict = get_type_dict("fenix_device")
    ...
    client.execute("INSERT INTO fenix_device ({cols}) VALUES".format(cols=cols), data, types_check=True)
