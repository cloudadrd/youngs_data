...
def delete_mysql(day,before_day) :
    db = pymysql.connect(**mysql_config)
    cursor = db.cursor()
    conn = cursor.execute
    delete_now= "delete from new_ssp.youngs_hour_report where time_slot='{time_slot}'".format(time_slot=day)
    delete_before= "delete from new_ssp.youngs_hour_report where time_slot like '{before_day}%'".format(before_day=before_day)
...
def build_insert_data_sql(day,hour):

        day = day + hour
        sql = """
            SELECT '{time_slot}' time_slot
...
GROUP BY multiIf(length(click_tb.country) > 1 ,click_tb.country ,length(conversion_tb.country) > 1 ,conversion_tb.country, length(reject_tb.country) > 1 ,conversion_tb.country ,'')
    ...
    , multiIf(length(click_tb.platform) > 1 ,click_tb.platform ,length(conversion_tb.platform) > 1 ,conversion_tb.platform, length(reject_tb.platform) > 1 ,conversion_tb.platform ,'')
            ...
            """.format(time_slot=day,hour=hour)
        print(sql)
        result = client.execute(sql)
        columns=['time_slot','country','offer','channel','pkg','platform','click_all','impression','install_buzz','conv','reject','revenue','cost']
        ...
        pd.io.sql.to_sql(df, 'youngs_hour_report', conn, schema='new_ssp', if_exists='append', index=False)
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        day = str((datetime.today() - timedelta(hours=inter)).strftime('%Y%m%d'))
        hour = str((datetime.today() - timedelta(hours=inter)).strftime('%H'))

    else:
        day = str((datetime.today() - timedelta(hours=1) ).strftime('%Y%m%d'))
        hour = str((datetime.today() - timedelta(hours=1)).strftime('%H'))
    delete_day = str((datetime.today() - timedelta(days=14) ).strftime('%Y%m%d'))
    delete_mysql(day + hour ,delete_day)
    clickhouse_df = build_insert_data_sql(day,hour)